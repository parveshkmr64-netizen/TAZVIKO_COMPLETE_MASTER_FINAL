(function(){
 const api=async(path,opt={})=>{const token=localStorage.getItem('tazviko_token')||'';const r=await fetch('/api/v1'+path,{headers:{'Content-Type':'application/json',...(token?{'Authorization':'Bearer '+token}:{})},...opt});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||('HTTP '+r.status));return j};
 ...
 window.placeDemoOrder=async function(){
   ...
   if(!name||!mobile||!address){notify('Please fill customer name, mobile and full delivery address');return}
   ...
   const btn=document.getElementById('placeOrderBtn'); const oldText=btn?btn.textContent:''; if(btn){btn.disabled=true;btn.textContent='Placing order...'}
   try{
     ...
     if(method==='COD'){clearAfterOrder(out.order_code);notify('COD order '+out.order_code+' placed successfully');return}
   ...
   }catch(e){notify('Order failed: '+e.message)}finally{if(btn){btn.disabled=false;btn.textContent=oldText||'Place Order'}}
 };
