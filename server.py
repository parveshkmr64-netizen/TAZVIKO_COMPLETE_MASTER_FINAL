...
def init_db():
    ...
    for name,typ in [('latitude','REAL'),('longitude','REAL'),('google_place_id','TEXT'),('source',"TEXT NOT NULL DEFAULT 'TAZVIKO'")]:
...
def calculate_order(items,coupon_code=''):
    ...
    try:
        for raw in items:
            ...
            if row is None: raise ValueError('product_not_available')
            line=int(row['price'])*qty; subtotal+=line
            normalized.append({'product_key':row['product_key'],'name':row['name'],'merchant':row['merchant'],'price':int(row['price']),'qty':qty,'line_total':line})
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p=='/api/v1/config': return self.send_json({'app_name':'TAZVIKO','currency':'INR','online_payment_enabled':bool(RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET),'razorpay_key_id':RAZORPAY_KEY_ID,'nearby_discovery_enabled':bool(GOOGLE_PLACES_API_KEY),'rider_portal':'/rider.html'})
        if p=='/api/v1/catalog':
            c=db(); rows=[dict(r) for r in c.execute('SELECT product_key,name,merchant,price FROM products WHERE active=1 ORDER BY merchant,name')]; c.close(); return self.send_json({'products':rows})
        if p=='/api/v1/auth/me':
    ...
    def do_POST(self):
        ...
        if p=='/api/v1/orders':
            ...
            c=db(); c.execute('''INSERT INTO orders(order_code,created_at,user_id,customer_name,mobile,address,merchant,items_json,subtotal,discount,delivery_fee,platform_fee,tax,total,commission,merchant_payable,tazviko_earning,payment_method,payment_status,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',vals); c.commit(); c.close(); return self.send_json({'ok':True,'order_code':code,'status':'PLACED','payment_status':pstatus,'quote':q},201)
        ...
        if p=='/api/v1/partners/applications':
            if not data.get('business_name'):return self.send_json({'error':'business_name_required'},400)
            c=db(); cur=c.execute('''INSERT INTO partners(created_at,business_name,owner_name,business_type,city,address,phone,bank_ref,catalog,status,latitude,longitude,google_place_id,source) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(now(),str(data.get('business_name',''))[:150],str(data.get('owner_name',''))[:120],str(data.get('business_type',''))[:80],str(data.get('city',''))[:120],str(data.get('address',''))[:500],str(data.get('phone',''))[:30],str(data.get('bank_ref',''))[:160],str(data.get('catalog',''))[:5000],'REVIEW',data.get('latitude'),data.get('longitude'),str(data.get('google_place_id',''))[:200],str(data.get('source','TAZVIKO'))[:30])); c.commit(); pid=cur.lastrowid; c.close(); return self.send_json({'ok':True,'id':pid,'status':'REVIEW'},201)
